    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <Pressable
          ...
          onPress={() =>
            router.push(`/(cleaner)/jobs/${item.bookingId}/chat` as never)
          }
        >
          <Avatar.Text
            size={44}
          ...
          <View style={styles.info}>
            ...
            <Text
              style={[styles.service, { color: theme.colors.primary }]}
              variant="labelSmall"
            >
              {item.serviceName}
            </Text>
      ...
      ListEmptyComponent={
        <EmptyState
          ...
          subtitle="Messages will appear once you accept a job"
        />
      }
    />
  );
...
const styles = StyleSheet.create({
  ...
  info: { flex: 1 },
  name: { fontWeight: "600" },
  service: { fontSize: 11, marginTop: 1 },
  preview: { marginTop: 2 },
  time: {},
});
